...
// GeminiChatImageHandler adapts Gemini image-preview models, which return
// generated images as inlineData parts from generateContent, to the OpenAI
// image response consumed by the image-generation UI.
func GeminiChatImageHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	revisedPrompt := geminiResponseUsageText(&geminiResponse)
	openAIResponse := dto.ImageResponse{
		Created: common.GetTimestamp(),
	...
	if len(openAIResponse.Data) == 0 {
		...
		return nil, types.NewOpenAIError(errors.New("no images generated by Gemini"), types.ErrorCodeBadResponseBody, http.StatusInternalServerError)
	}

	usage := buildUsageFromGeminiResponse(c, info, &geminiResponse)
	if usage.CompletionTokens == 0 {
		patchGeminiZeroCompletionUsage(c, info, &usage, revisedPrompt, len(openAIResponse.Data))
	}
